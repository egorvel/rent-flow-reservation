...
class ReservationIT extends PostgresIntegrationTest {
    ...
    class CreationBasics {
        ...
        void oneSuccessfulBatchUsesOneDatabaseCreationTimeAndConfiguredDeadline() throws Exception {
            ...
            assertThat(created.get(1).path("timestamp"))
                    .isEqualTo(created.get(0).path("timestamp"));
    ...
    class CreationPolicies {
        ...
        void storesAndReplaysOtherTerminalInventoryFailures(String type, int statusCode, String code) throws Exception {
            ...
            List<Failure> failures = resultType == ClaimResult.Type.KEY_REUSED
                    ? List.of()
                    : List.of(new Failure(
                            0,
                            "DRILL-001",
        ...
        void resumesTemporaryInventoryFailureWithTheSameKey() throws Exception {
            ...
            String body = batch(CREATE);
            when(inventoryGateway.claim(any(), any()))
                    .thenThrow(new ServiceUnavailableException(new IllegalStateException()))
        ...
        void retriesProtocolAmbiguityOnlyWhenTheClientRetries() throws Exception {
            UUID key = UUID.randomUUID();
            String body = batch(CREATE);
            when(inventoryGateway.claim(any(), any())).thenThrow(new ProtocolException(new IllegalStateException()));
        ...
        void configuresFiveExponentialJitteredForegroundAttempts() {
            ...
            for (int attempt = 1; attempt <= 4; attempt++) {
                long wait =
                        retryConfig.getIntervalBiFunction().apply(attempt, Either.left(new IllegalStateException()));
        ...
        void cleanupDeletesOnlyExpiredLedgerRows() throws Exception {
            ...
            assertThat(cleanup.deleteExpiredCreationChunk()).isOne();
    ...
    class Crud {
        ...
        void replacementUpdatesAllMutableFieldsAndPreservesManagedFields(String newStatus) throws Exception {
            ...
            """.formatted(newStatus);
            JsonNode updated =
                    body(mvc.perform(put(PATH + "/" + original.path("id").asString())
                                    .contentType(MediaType.APPLICATION_JSON)
                                    .content(replacement))
    ...
    class Collection {
        ...
        void sortsAllFieldsDeterministically(String field, String direction) throws Exception {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                expected.add(
                        body(mvc.perform(put(PATH + "/" + created.path("id").asString())
                                        .contentType(MediaType.APPLICATION_JSON)
                                        .content(replacement))
                                .andExpect(status().isOk())));
            }
            ...
            JsonNode page = body(mvc.perform(get(PATH)
                            .param("sort", field)
                            .param("direction", direction.toUpperCase(java.util.Locale.ROOT)))
                    .andExpect(status().isOk()));
